...
# Path to image
input_path = '/Users/bhsieh/Desktop/Penn Air/PennAir 2024 App Dynamic.mp4'
output_path = '/Users/bhsieh/Desktop/Penn Air/Part 2 Processed Video.mp4'
...
while video_capture.isOpened():
    ...
    if ret:
        ...
        average_color = np.round(average_color_per_channel).astype(int) + 35

        result = frame.astype(int) - average_color
        result = np.clip(result, 0, 255).astype(np.uint8)

        lower_bound = np.array([0, 0, 0], dtype=np.uint8)
        upper_bound = np.array([35, 35, 35], dtype=np.uint8)
        ...
        fixed_contours = [contour for contour in contours if cv.contourArea(contour) > 100]
